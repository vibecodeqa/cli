...
function issueMeta(iss: Issue): string {
	const parts = [iss.wcag, iss.selector, iss.suggestion].filter(Boolean) as string[];
	if (parts.length === 0) return "";
	return `<span class="iru">${e(parts.join(" · "))}</span>`;
}
		...
		.map((c) => {
			...
			for (const [file, issues] of byFile) {
				...
				for (const iss of issues) {
					...
					issuesHtml += `<div class="ir ${iss.severity}"><span class="is">${iss.severity[0]!.toUpperCase()}</span>${iss.line ? `<span class="il">${iss.line}</span>` : ""}<span class="im">${e(iss.message)}</span>${iss.rule ? `<span class="iru">${e(iss.rule)}</span>` : ""}${issueMeta(iss)}${snippetBtn}<button class="cp-btn" data-prompt="${e(prompt)}" title="Copy fix prompt">\ud83d\udccb</button></div>${srcBlock}`;
				}
				issuesHtml += `</div>`;
			}
			if (noFile.length > 0) {
				...
				for (const iss of noFile) {
					issuesHtml += `<div class="ir ${iss.severity}"><span class="is">${iss.severity[0]!.toUpperCase()}</span><span class="im">${e(iss.message)}</span>${iss.rule ? `<span class="iru">${e(iss.rule)}</span>` : ""}${issueMeta(iss)}</div>`;
				}
				issuesHtml += `</div>`;
			}
...
export function issuesPage(allChecks: CheckResult[], totalIssues: number, fl: FL): string {
	...
	const issueRows = allIssues
		...
		.map((i) => {
			const loc = i.file && typeof i.file === "string" ? fl(i.file.split(":")[0]!, i.line) : "";
			const msg = `${i.message}${i.suggestion ? ` Fix: ${i.suggestion}` : ""}`;
			const rule = [i.rule, i.wcag, i.selector].filter(Boolean).join(" · ");
			return `<tr class="${i.severity}"><td class="is2">${i.severity[0]!.toUpperCase()}</td><td class="ic2">${e(i.check)}</td><td class="il2">${loc}</td><td>${e(msg)}</td><td class="iru2">${e(rule)}</td></tr>`;
		})
		.join("");
